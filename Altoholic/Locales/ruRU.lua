...
-- ** Общий доступ к аккаунту **
L["Send Request"] = "Отправить запрос"
L["Request Content"] = "Запросить данные"
L["Getting table of content from %s"] = "Получение списка содержимого от %s"
L["Transfering item %d/%d"] = "Передача элемента %d/%d"
L["Transfer interrupted"] = "Передача прервана"
L["No answer from %s"] = "Нет ответа от %s"
L["Opens the account sharing panel"] = "Открывает панель общего доступа к аккаунту"
L["Shared Content"] = "Общие данные"
L["ACCOUNT_NAME_IDENTIFIES"] = "Обозначает аккаунт, |cFF00FF00из которого|r вы\nимпортируете данные, например имя игрока."

L["SHARING_HELP_SEND_REQUEST"] = "|cFFFFD7001) Имя аккаунта:|cFFFFFFFF любая метка на ваш выбор для аккаунта, |cFF00FF00из которого|cFFFFFFFF вы хотите импортировать данные (например, имя или прозвище\nигрока). Она лишь группирует импортированных персонажей на вкладке «Сводка» и может быть любой.\nВ следующий раз введите то же имя: импорт обновит эту группу, а не добавит второй аккаунт.\n\n|cFFFFD7002) Отправить запрос:|cFFFFFFFF возьмите другого игрока в цель или введите имя персонажа, |cFF00FF00за которого он играет сейчас|cFFFFFFFF, затем нажмите кнопку.\nНа той стороне должен быть включён общий доступ, и запрос должен быть принят, либо ваш персонаж уже должен быть авторизован.\nПосле принятия запроса всё, что открыто на той стороне, появится справа, а эта кнопка станет «Запросить данные»."
L["SHARING_HELP_REQUEST_CONTENT"] = "|cFF00FF00Запрос принят.|cFFFFFFFF То, что открывает этот игрок, теперь перечислено справа, но ещё ничего не импортировано.\nОтметьте нужных персонажей и данные (отметка персонажа отмечает и его категории данных) либо используйте\nкнопки [-] и «Всё» над списком, чтобы развернуть или отметить всё, затем нажмите «Запросить данные».\n\nСтолбец даты показывает возраст каждого элемента, «Актуально» означает, что эта версия у вас уже есть."
L["SHARING_HELP_TRANSFER_IN_PROGRESS"] = "|cFFFFFFFFИдёт передача, подождите...\nОба персонажа должны оставаться в сети до её завершения."
L["SHARING_HELP_TRANSFER_COMPLETE"] = "|cFF00FF00Передача завершена.|cFFFFFFFF Импортированные персонажи теперь на вкладке «Сводка», сгруппированы под введённым вами именем аккаунта.\nСписок справа очищен, это нормально.\n\n|cFFFFD700Примечание:|cFFFFFFFF импорт — это разовый снимок, он |cFF00FF00не|cFFFFFFFF обновляется автоматически.\nЧтобы обновить его позже, щёлкните правой кнопкой по строке игрового мира на вкладке «Сводка» и выберите «Обновить из ...», либо вернитесь сюда.\nДанные может передать любой персонаж этого аккаунта, так что спрашивайте того, кто в сети, и используйте то же имя аккаунта."
L["SHARING_HELP_REQUEST_SENT"] = "|cFFFFD700Запрос отправлен.|cFFFFFFFF Ожидание ответа от %s...\nЗапрос должен быть принят на той стороне, если только ваш персонаж уже не авторизован там.\nЕсли ничего не происходит, проверьте, что %s в сети, на вашем игровом мире и использует Altoholic."
L["SHARING_HELP_REQUEST_REJECTED"] = "|cFFFF0000Запрос отклонён.|cFFFFFFFF Этот игрок либо отклонил ваш запрос, либо установил для вашего персонажа «всегда отклонять».\nНичего не импортировано. Вы можете отправить новый запрос в любой момент."
L["SHARING_HELP_REQUEST_CANCELLED_COMBAT"] = "|cFFFF0000Запрос отменён.|cFFFFFFFF Запросы общего доступа всегда отклоняются, пока другой игрок в бою.\nДождитесь окончания боя и отправьте запрос снова."
L["SHARING_HELP_SHARING_DISABLED"] = "|cFFFF0000Запрос отклонён.|cFFFFFFFF Этот игрок не включил общий доступ к аккаунту.\nПараметр «Общий доступ к аккаунту включён» должен быть отмечен в настройках Altoholic другого игрока, прежде чем вы сможете что-либо запросить."
L["SHARING_HELP_TRANSFER_INTERRUPTED"] = "|cFFFF0000Передача прервана.|cFFFFFFFF %s перестал отвечать, поэтому запрос был прекращён.\n%s\nКнопка снова активна: проверьте, что другой персонаж всё ещё в сети, затем отправьте новый запрос."
L["SHARING_HELP_NO_ANSWER"] = "|cFFFF0000Нет ответа.|cFFFFFFFF %s так и не ответил на запрос.\nПроверьте, что этот персонаж в сети, на вашем игровом мире, использует Altoholic и что общий доступ включён на той стороне.\nНичего не импортировано. Вы можете отправить новый запрос в любой момент."
L["SHARING_PARTIAL_IMPORT_KEPT"] = "То, что уже было передано, сохранено, поэтому эти персонажи есть на вкладке «Сводка»: отправьте запрос снова с тем же именем аккаунта, чтобы завершить их."
L["SHARING_PARTIAL_IMPORT_NONE"] = "Ещё ничего не было передано, поэтому ничего не импортировано."

-- ** Поиск по таблицам добычи **
L["SEARCHING_UPGRADES"] = "Поиск улучшения по сравнению с %s в таблицах добычи... игра замрёт на несколько секунд."
L["SEARCH_RESULTS"] = "Найдено результатов: %d"
L["SEARCH_RESULTS_INCOMPLETE"] = "Результатов: %d - %d названий пока неизвестны"
L["SEARCH_IN_PROGRESS"] = "Поиск по таблицам добычи... %d%%"
L["SEARCH_ALREADY_RUNNING"] = "Поиск уже выполняется, дождитесь его завершения."

-- ** Снаряжение членов гильдии **
L["EQUIPMENT_COMES_FROM_MEMBER"] = "Снаряжение присылает его собственная копия аддона, поэтому он должен быть в сети."
L["EQUIPMENT_ASKING"] = "Запрос к %s..."
L["EQUIPMENT_NO_ANSWER"] = "%s не ответил"
L["EQUIPMENT_RECEIVED_ON"] = "Получено %s"
